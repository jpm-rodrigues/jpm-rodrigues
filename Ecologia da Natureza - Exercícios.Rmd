...
df <- data.frame(mês = c("Jan", "Fev", "Mar", "Abr", "Mai", "Jun", "Jul", "Ago", "Set", "Out", "Nov", "Dez"), cidade_do_mexico = c(10.2, 10.2, 12.7, 27.9, 58.4, 157.5, 182.9, 172.7, 144.8, 61, 5.1, 0.8), quito = c(114.3, 129.5, 152.4, 175.3, 124.5 , 48.3, 20.3, 25.4, 78.7, 127, 109.2, 104.1), la_paz = c(129.5, 104.1, 71.1, 35.6, 12.7, 5.1, 6.7, 15.2, 30.5, 40.6, 50.8, 94))
...
df2$mês <- factor(df2$mês, levels = c("Jan", "Fev", "Mar", "Abr", "Mai", "Jun", "Jul", "Ago", "Set", "Out", "Nov", "Dez"))

# Criango o gráfico
ggplot(df2, aes(x = mês, y = precipitação, fill = cidades)) +
         geom_col(position = "dodge") # Barras lado ao lado
```
...
ggplot(df, aes(mês, temp, group = 1)) + 
  geom_bar(aes(y = precipitação/2), stat = "identity") + 
...
A tabela a seguir lista as distribuições das frequências dos tamanhos dos bicos dos tentilhões, tanto antes quanto depois da seleção. Utilizando um gráfico de barras,
plote as relações entre tamanho do bico e sua frequência; então, determine quanto o tamanho médio do bico mudou devido à seleção e decida qual tipo de seleção
ocorreu. 
...
# Criando o gráfico
ggplot(df, aes(anos, proporção, color = sexo)) +
geom_line() +
theme_minimal()
```
